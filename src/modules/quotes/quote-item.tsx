...
import { Package, CheckCircle, Clock, XCircle, CreditCard, DollarSign, Info, FileText, User } from "lucide-react"
import { formatCurrency } from "@/lib/formatCurrency"
import { type Cotizacion, EstatusCotizacion, MetodoPago } from "@/interfaces/cotizaciones/cotizacion.interface"
      ...
      <div className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-full ${getStatusColor(cotizacion.estatus)}`}>
          ...
          <Badge
            variant="outline"
            className={`capitalize ${getStatusColor(cotizacion.estatus)} border-0 flex gap-2 p-1.5`}
          >
            {getStatusIcon(cotizacion.estatus)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="flex items-start gap-2">
            <div className="flex items-center gap-1">{getPaymentMethodIcon(cotizacion.metodoPago)}</div>
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400">Método de pago</p>
              <p className="text-sm capitalize">{cotizacion.metodoPago.toLowerCase()}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-sm">
                {cotizacion.productos.length} {cotizacion.productos.length === 1 ? "artículo" : "artículos"}
              </p>
            </div>
          </div>
                  ...
                  {cotizacion.productos.map((producto) => (
                    ...
                    >
                      <div className="flex-shrink-0">
                        <Image
                          src={producto.producto.coverUrl || "/imgs/products/default-img.png"}
                      ...
                      <div className="flex-grow">
                        ...
                        {/* <p className="text-sm text-slate-500 dark:text-slate-400">
                          {producto.cantidad} × {formatCurrency(producto.precioDescuento)}
                        </p> */}
                      </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {cotizacion.notaVendedor && (
                      <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
                        ...
                        <p className="text-sm text-green-700 dark:text-green-200">{cotizacion.notaVendedor}</p>
                      </div>
                    )}
                  </div>
                )}
